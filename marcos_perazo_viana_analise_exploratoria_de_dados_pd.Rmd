...
```{r densidade original, echo=TRUE}
dados %>% dplyr::select(Peso) %>% ggplot(aes(x=Peso, y = after_stat(density))) + geom_density(linetype = 2) + xlab('Peso') + ylab('Densidade de Frequência') + theme_classic()
```

```{r densidade após inclusão, echo=TRUE}
dados_imputados %>% dplyr::select(Peso) %>% ggplot(aes(x=Peso, y = after_stat(density))) + geom_density(linetype = 2) + xlab('Peso') + ylab('Densidade de Frequência') + theme_classic()
```
